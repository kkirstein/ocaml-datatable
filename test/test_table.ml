...
let data_series_to_string s =
  ...
  Printf.sprintf "{name: %s; type: %s; length: %d}" sum.name sum.data_type sum.length

let data_series = Alcotest.testable (Fmt.of_to_string data_series_to_string) (=)
...
(* ---------------------------------------------------------------------- *)
let test_get_col () =
  let exp_str = SStr (Strings.from_list ~name:"order" ["eins"; "zwei"; "drei"]) in
  let exp_float = SFloat (Floats.from_list ~name:"values" [1.47; 2.71; 3.14]) in
